...
func updateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	param, _ := strconv.Atoi(mux.Vars(r)["id"])
	var customer Customer
	result := db.First(&customer, param)
	if result.Error != nil {
		http.Error(w, `{"error": "Customer details not found"}`, http.StatusNotFound)
	...
	err := json.NewDecoder(r.Body).Decode(&latestDetail) //alternate to decode the JSON request to Go data structure
	if err != nil {
		http.Error(w, "Invalid JSON data", http.StatusBadRequest)
		return
	}

	latestDetail.Id = customer.Id //avoid updating ID value

	if err := db.Model(&Customer{}).Where("id = ?", param).Updates(latestDetail).Error; err != nil {
		http.Error(w, "Error while updating the details", http.StatusInternalServerError)
		return
	}

	w.WriteHeader(http.StatusAccepted)
	w.Write([]byte(`{"message": "Customer details updated successfully"}`))
}
...
func deleteCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if err := db.Delete(&Customer{}, "id = ?", param).Error; err != nil {
		http.Error(w, "Error deleting the data from the database", http.StatusInternalServerError)
		return
	}
